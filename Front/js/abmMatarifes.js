...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    // -------------------------------------------------------------------------
    const $form = document.getElementById("formMatarifes");
    if ($form) {
    ...
    function limpiarValidacionForm() {
        document.querySelectorAll('#formMatarifes .is-invalid')
    ...
    async function obtenerMatarifes() {
        try {
            ...
            console.error("Error obteniendo matarifes:", error);
            return [];
        }
    }
    ...
    async function inicializarTabla() {
        let data = await obtenerMatarifes();
        tablaMatarifes = $("#tablaMatarifes").DataTable({
            ...
            data: data,
            columns: [
                // IMPORTANTE: ajustá el nombre del ID si tu API usa otro (por ej. "id" o "id_matarife")
                { data: "idMatarife" },
                { data: "cuit" },
                { data: "razón_social" },
                { data: "teléfono" },
    ...
    async function recargarTabla() {
        let data = await obtenerMatarifes();
        tablaMatarifes.clear().rows.add(data).draw();
    }
    ...
    document.getElementById("btnCrear").addEventListener("click", function () {
        opcion = "crear";
        document.getElementById("formMatarifes").reset();
        limpiarValidacionForm();
        document.querySelector(".modal-title").textContent = "Crear Matarife";
    ...
    // -------------------------------------------------------------------------
    document.getElementById("tablaMatarifes").addEventListener("click", function (event) {
        ...
        if (target && target.classList.contains("btnEditar")) {
            ...
            let data = tablaMatarifes.row(fila).data();

            // IMPORTANTE: si tu API no usa "idMatarife", cambiá estas líneas por el nombre correcto.
            document.getElementById("id").value = data.idMatarife;
            ...
            document.querySelector(".modal-title").textContent = "Editar Matarife";
            new bootstrap.Modal(document.getElementById("modalCRUD")).show();
        }
    });
    ...
    // -------------------------------------------------------------------------
    document.getElementById("formMatarifes").addEventListener("submit", async function (e) {
        e.preventDefault();
        ...
        let datos = {
            // IMPORTANTE: si tu backend usa otro nombre de ID, cambiá aquí también.
            idMatarife: document.getElementById("id").value.trim(),
        ...
        let endpoint = opcion === "crear" ? url : url + datos.idMatarife;
   ...
   // Botón Borrar
document.getElementById("tablaMatarifes").addEventListener("click", async function (event) {
    let target = event.target;
    if (target.tagName === "I") {
        target = target.closest("button");
    }
    ...
    if (target && target.classList.contains("btnBorrar")) {
        let fila = target.closest("tr");
        let data = tablaMatarifes.row(fila).data();
        let idMatarife = data.idMatarife; // asegurate que este sea el nombre correcto
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (!response.ok) {
                        // Si hay error (por restricción FK o cualquier otro)
    ...
    document.getElementById("tablaMatarifes").addEventListener("click", function (event) {
        ...
        if (target && target.classList.contains("btnVer")) {
            ...
            document.getElementById("verID").textContent = data.idMatarife; // ajustar si tu API usa otro nombre
