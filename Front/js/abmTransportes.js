...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    // -----------------------------------------------------------------------------
    const $form        = document.getElementById("formTransportes");
    ...
    // -------------------------------------------------------------------------
    // Verificación de datos en tiempo real (sanitizado)
    //  - CUIT: ya tenés oninput en HTML dejando solo dígitos y 11 máx.
    ...
    // // Quitar .is-invalid del form
    function limpiarValidacionForm() {
        document.querySelectorAll('#formTransportes .is-invalid')
    ...
    async function inicializarTabla() {
        let data = await obtenerTransportes();
    ...
    async function recargarTabla() {
        let data = await obtenerTransportes();
        tablaTransportes.clear().rows.add(data).draw();
    }
    ...
    document.getElementById("btnCrear").addEventListener("click", function () {
        opcion = "crear";
        document.getElementById("formTransportes").reset();
        limpiarValidacionForm(); // limpiar rojos previos
        document.querySelector(".modal-title").textContent = "Crear Transporte";
    ...
    // -----------------------------------------------------------------------------
    document.getElementById("tablaTransportes").addEventListener("click", function (event) {
        ...
        if (target && target.classList.contains("btnEditar")) {
            ...
            let data = tablaTransportes.row(fila).data();

            document.getElementById("id").value = data.idTransporte;
            ...
            document.querySelector(".modal-title").textContent = "Editar Transporte";
            new bootstrap.Modal(document.getElementById("modalCRUD")).show();
        }
    });
    ...
    // -----------------------------------------------------------------------------
    document.getElementById("formTransportes").addEventListener("submit", async function (e) {
        e.preventDefault();
        ...
        let datos = {
            idTransporte: document.getElementById("id").value.trim(),
        ...
        let endpoint = opcion === "crear" ? url : url + datos.idTransporte;
        ...
        try {
            ...
            Swal.fire({ icon: "error", title: "No se pudo guardar", text: "Intentá de nuevo." });
        }
    });

// Botón Borrar
document.getElementById("tablaTransportes").addEventListener("click", async function (event) {
    let target = event.target;
    if (target.tagName === "I") {
        target = target.closest("button");
    }
    ...
    if (target && target.classList.contains("btnBorrar")) {
        let fila = target.closest("tr");
        let data = tablaTransportes.row(fila).data();
        let idTransporte = data.idTransporte;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (!response.ok) {
                        // Error (FK, inexistente o general)
    ...
    document.getElementById("tablaTransportes").addEventListener("click", function (event) {
        ...
        if (target && target.classList.contains("btnVer")) {
            ...
            document.getElementById("verID").textContent = data.idTransporte;
